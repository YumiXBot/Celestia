...
@Celestia.on_message(filters.command("broadcast") & filters.user(OWNER_ID))
...
@Celestia.on_message(filters.command("announce") & filters.user(OWNER_ID))
async def announced(_, message):
    ...
    for chat in chats:
      try:
        await Celestia.forward_messages(chat_id=int(chat), from_chat_id=message.chat.id, message_ids=to_send)
    ...
    for user in users:
      try:
        await Celestia.forward_messages(chat_id=int(user), from_chat_id=message.chat.id, message_ids=to_send)
        await asyncio.sleep(1)
