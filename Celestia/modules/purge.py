...
@Celestia.on_message(filters.command("purge") & admin_filter)
async def purge(celestia: Celestia, msg: Message):
    ...
    if msg.reply_to_message:
        ...
        try:
            for plist in m_list:
                await celestia.delete_messages(chat_id=msg.chat.id, message_ids=plist, revoke=True)
...
@Celestia.on_message(filters.command("spurge") & admin_filter)
async def spurge(celestia: Celestia, msg: Message):
...
@Celestia.on_message(filters.command("del") & admin_filter)
async def del_msg(celestia: Celestia, msg: Message):
    if msg.chat.type != ChatType.SUPERGROUP:
        await msg.reply_text(text="**ɪ ᴄᴀɴ'ᴛ ᴘᴜʀɢᴇ ᴍᴇssᴀɢᴇs ɪɴ ᴀ ʙᴀsɪᴄ ɢʀᴏᴜᴘ ᴍᴀᴋᴇ sᴜᴘᴇʀ ɢʀᴏᴜᴘ.**")
        return        
    if msg.reply_to_message:
        await msg.delete()
        await celestia.delete_messages(chat_id=msg.chat.id, message_ids=msg.reply_to_message.id)
    else:
