...
timeout = {"timeout": 40}  # Specify timeout as a dictionary

http = httpx.AsyncClient(http2=True, **timeout)
...
@Celestia.on_message(filters.command("weather"))
async def weather_command(c: Celestia, m: Message):
    if len(m.command) == 1:
