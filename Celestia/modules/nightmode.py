...
from Celestia.Helper.database.nightmodedb import nightdb,nightmode_on,nightmode_off,get_nightchats 
...
@Celestia.on_message(filters.command("nightmode") & filters.group)
...
@Celestia.on_callback_query(filters.regex("^(add_night|rm_night)$"))
async def nightcb(_, query : CallbackQuery):
    ...
    administrators = []
    async for m in Celestia.get_chat_members(chat_id, filter=enums.ChatMembersFilter.ADMINISTRATORS):
...
async def start_nightmode() :
    ...
    for add_chat in chats:
        try:
            ...
            await Celestia.set_chat_permissions(add_chat,CLOSE_CHAT)
...
async def close_nightmode():
    ...
    for rm_chat in chats:
        try:
            ...
            await Celestia.set_chat_permissions(rm_chat,OPEN_CHAT)
